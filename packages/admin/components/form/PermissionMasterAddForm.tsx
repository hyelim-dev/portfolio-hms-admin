...
export default function PermissionMasterAddForm({ permission }) {
  ...
  const grantSubMasters = async id => {
    ...
    if (!result.data.editManageUser.ok) {
      throw new Error(`${permission.permissionName} 부운영자지정 실패`)
    }
  }
    ...
    if (isDirty) {
      ...
      if (isModify) {
        try {
          ...
          //   `ok: ${resultData.editPermissionsGranted.ok} , managers: ${managers}`,
          // )
          ...
          await Promise.all(
            managersID.map(async id => {
              try {
                await grantSubMasters(id)
              } catch (error) {
                console.error(
                  `managerID: ${id} 부운영자 지정 중 에러 발생:`,
                  error,
                )
              }
            }),
